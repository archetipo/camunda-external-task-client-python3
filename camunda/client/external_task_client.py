...
class ExternalTaskClient:
    ...
    def __init__(self, worker_id, engine_base_url=ENGINE_LOCAL_BASE_URL,
                 config=None):
        ...
        self._log_with_context(
            f"Created External Task client with config: {obfuscate_password(self.config)}")
    ...
    def fetch_and_lock(self, topic_names, process_variables=None, variables=None):
        ...
        if self.is_debug:
            self._log_with_context(
                f"trying to fetch and lock with request payload: {body}")
        ...
        if self.is_debug:
            self._log_with_context(
                f"fetch and lock response json: {resp_json} for request: {body}")
    ...
    def __get_fetch_and_lock_http_timeout_seconds(self):
        ...
        return (
                self.config["timeoutDeltaMillis"] + self.config["asyncResponseTimeout"]
        ) / 1000
    ...
    def _get_topics(self, topic_names, process_variables, variables):
        ...
        for topic in str_to_list(topic_names):
            topics.append({
                ...
                "includeExtensionProperties": self.config.get(
                    "includeExtensionProperties") or False,
    ...
    def failure(self, task_id, error_message, error_details, retries,
                retry_timeout):
    ...
    def bpmn_failure(self, task_id, error_code, error_message, variables=None):
        ...
        if self.is_debug:
            self._log_with_context(
                f"trying to report bpmn error with request payload: {body}")
    ...
    def auth_basic(self) -> dict:
        if not self.config.get("auth_basic") or not isinstance(
                self.config.get("auth_basic"), dict):
            return {}
