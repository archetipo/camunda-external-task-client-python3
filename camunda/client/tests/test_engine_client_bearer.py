...
class EngineClientAuthTest(TestCase):
    ...
    def test_auth_basic_start_process_success(self):
        ...
        actual_resp_payload = self.client.start_process(self.process_key, {},
                                                        self.tenant_id,
                                                        "123456")
    ...
    def test_auth_basic_start_process_server_error_raises_exception(self):
        ...
        print(str(exception_ctx.exception))
        self.assertTrue(
    ...
    def test_auth_basic_get_process_instance_success(self):
        ...
        responses.add(responses.GET, get_process_instance_url,
    ...
    @responses.activate
    def test_auth_basic_get_process_instance_bad_request_raises_exception(
            self):
    ...
    @responses.activate
    def test_auth_basic_get_process_instance_server_error_raises_exception(
            self):
        get_process_instance_url = f"{ENGINE_LOCAL_BASE_URL}/process-instance" \
                                   f"?processDefinitionKey={self.process_key}" \
                                   f"&tenantIdIn={self.tenant_id}" \
    ...
    @patch('requests.post')
    def test_auth_basic_correlate_message_with_only_message_name(self,
                                                                 mock_post):
    ...
    @responses.activate
    def test_auth_basic_correlate_message_invalid_message_name_raises_exception(
            self):
        ...
        responses.add(responses.POST, correlate_msg_url,
                      status=HTTPStatus.BAD_REQUEST, json=resp_payload)
        with self.assertRaises(Exception) as exception_ctx:
    ...
    def test_auth_basic_get_process_instance_variable_without_meta(self):
        ...
        responses.add(responses.GET, process_instance_var_url,
                      status=HTTPStatus.OK, json=resp_frame_payload)
        responses.add(responses.GET, process_instance_var_data_url,
                      status=HTTPStatus.OK, body=resp_data_payload)

        resp = self.client.get_process_instance_variable(process_instance_id,
                                                         variable_name)
